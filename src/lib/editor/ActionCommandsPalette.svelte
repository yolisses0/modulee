...
<div
	bind:this={div}
	class="fixed top-0 right-0 left-0 m-auto flex max-h-[75vh] max-w-sm flex-col gap-2 rounded border border-black bg-zinc-800 p-2"
>
	<input type="text" bind:value={text} class="common-input" />
	<div class="overflow-auto">
		{#if values.length}
			<BasicList items={values} getId={getSame} {getName} onClick={handleClick} />
		{:else}
